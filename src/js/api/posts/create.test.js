...
const TEST_MEDIA = "https://cdn.pixabay.com/photo/2014/05/27/23/32/matrix-356024_960_720.jpg";
...
describe("createPost", () => {
  it("Returns a response object with corresponding values for inputs", async () => {
    ...
    const response = await createPost(TEST_TITLE, TEST_BODY, TEST_MEDIA, TEST_TAGS);
  ...
  it("Returns error message if request was bad", async () => {
    global.fetch = jest.fn(() => createBadRequest());
    let error;
    try {
      await createPost(TEST_TITLE, TEST_BODY, TEST_MEDIA, TEST_BAD_TAGS);
    } catch (e) {
      error = e;
    } finally {
  ...
  it("Returns error message if request was unauthorized", async () => {
